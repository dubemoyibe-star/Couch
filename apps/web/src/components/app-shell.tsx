...
export function AppShell({ name, children }: { readonly name: string; readonly children: ReactNode }) {
  return (
    <div className="flex min-h-dvh">
      <aside className="sticky top-0 hidden h-dvh w-64 shrink-0 p-3 md:block">
        <div className="flex h-full flex-col gap-6 rounded-lg border border-border bg-surface/60 p-4">
          ...
          <span
            aria-disabled="true"
            title="Coming soon"
            className="flex min-h-11 cursor-not-allowed items-center gap-3 rounded-md px-3 text-sm font-medium text-text-muted opacity-60"
          >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex flex-wrap items-center gap-3 px-4 pb-1 pt-3 sm:px-6 md:flex-nowrap md:justify-end md:pt-4">
          ...
          <div className="ml-auto flex items-center gap-3 md:order-last md:ml-0">
            {/* Notifications are not built yet; the bell is a placeholder. */}
            <button
              type="button"
              aria-label="Notifications"
              className={cx(
                "inline-flex size-10 cursor-pointer items-center justify-center rounded-full text-text-muted hover:bg-surface-muted hover:text-text",
                calmTransition,
                focusRing,
              )}
            >
