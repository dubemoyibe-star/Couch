...
A connection is in exactly one room at a time. `room.join` checks membership in the database (`getMembership`), then attaches the connection; a second `room.join` on the same connection is refused with `already_joined` and never moves it. Presence is derived, not stored: a user is online in a room while at least one of their connections is attached to it. Their first connection announces `room.memberJoined` to the others, a further connection sends `presence.update` (online), and the close of their last connection, graceful or not, sends `presence.update` (offline). A room is created in the store on first use, and only for a couch whose current media resolves through the license gate, since a room needs media.
