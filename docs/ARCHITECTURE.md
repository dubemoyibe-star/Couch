...
### Couch teardown (internal endpoint)

When a couch is deleted, apps/web asks the realtime service to end its live room. The realtime HTTP server, the same one that handles the WebSocket upgrade, serves `POST /internal/couches/:couchId/teardown`. It is reached by a plain HTTP request, never by a WebSocket upgrade, so a normal client cannot get to it.

- **Secret first**: the request must carry the shared secret in the `x-internal-secret` header, compared in constant time with `REALTIME_INTERNAL_SECRET`. This is checked before the path, the method, the couch id or the room store is looked at. Every request under `/internal/` without a valid secret gets the same empty 401, so a caller learns nothing about which paths exist or whether a room exists for a couch, the same anti-enumeration rule the WebSocket handshake follows for sessions. The service refuses to start without the secret, so the endpoint cannot be left open. The secret is never logged.
- **On a valid request**: every connection attached to the couch receives `room.deleted` and is closed with code 1000, and the room is removed from the `RoomStore`. The answer is an empty 204 whether or not there was anything to tear down, so a couch with no room and no connections is a safe no-op. Connections are torn down even when no room is stored, because a couch that never had media has connections and no room. No `room.memberLeft` or `presence.update` is sent, since the whole room ends. Other couches are untouched.
- **The one exception to rooms persisting**: rooms otherwise live for the whole process, even after their media is cleared, because the couch still exists. This is the only place a room is deleted from the store, and it is justified because the couch itself no longer exists, not because it has no media now. `RoomStore` gains `delete` for it.
- **Internal only**: the endpoint must never be exposed publicly. Keep it off any public route or proxy rule. It exists purely for service-to-service coordination between apps/web and apps/realtime.
- **Not a dependency violation**: "apps never import from each other" is a rule about module imports, meaning code dependencies. This endpoint is a runtime network call between two separate processes. apps/web does not import anything from apps/realtime, and the shapes both sides rely on live in packages such as `@couch/contracts`. The two rules do not conflict.
- **Streaming**: once local streaming exists, the teardown must also stop any active LiveKit stream for the room. A note at the teardown handler in `room-handlers.ts` marks the spot.

Configuration: `REALTIME_INTERNAL_SECRET` (both apps, the same value) and `REALTIME_INTERNAL_URL` (apps/web only), placeholders in `.env.example`.
